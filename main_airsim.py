...
MA_Y = collections.deque(maxlen=MAX_MA_Y_LEN)
...
class Mainflow:
    ...
    def prepare_visualisation(self, lidar_distance, person_center, person_to_track, image,
        ...
        cv2.line(image, (lidar_vis_x, lidar_vis_y), (lidar_vis_x, lidar_vis_y2), (0, 255, 0), thickness=5, lineType=8,
                 shift=0)
        ...
        # show drone center
        cv2.circle(image, (int(self.image_center[0]), int(self.image_center[1])), 20, (0, 255, 0), thickness=3, lineType=8,
                   shift=0)

        # show trackable center
        cv2.circle(image, (int(person_center[0]), int(person_center[1])), 20, (0, 0, 255), thickness=3, lineType=8,
                   shift=0)
    ...
    def track(self):
        ...
        while True:
            ...
            if len(detections) > 0 and len(detections.xyxy[0].values) > 0:
                ...
                # y_delta_MA = calculate_ma(MA_Y)
                # self.control.setYdelta(y_delta_MA)
                ...
                velocity_z_command = self.control.getMovementVelocityXCommand()
                # velocity_x_command = self.control.getMovementVelocityYCommand()

                # draw lidar distance
                self.prepare_visualisation(obj_dist, obj_center, obj_to_track, color, yaw_command, x_delta, y_delta, velocity_z_command)
            else:
                return "search"
